...
CITATION_LENGTH_MAX = configo.HV_INT_PLUS(default=50)

# TODO: SENTENCE END
# Ich kann mit Jugendlichen einen Spielplatz planen, aber nicht große europäische Politik machen.“.


def work(text: str, pages: tuple) -> protocol.ResultType:
    quotations = serializeraw.load_quotations(
        text,
        pages=pages,
    )
    driver = protocol.driver(quotations=quotations)
    result = protocol.run(
        __name__,
        driver=driver,
    )
...
def message(quotation: str, msgid=None):
    driver = protocol.driver(quotations=[
        iamraw.ExtractedQuotation(0, 0, quotation),
    ])
    user, _ = protocol.run(modulename=__name__, driver=driver)
