...
import textflow_.writing.utils
...
def determine(sentences) -> WritingStatistics:
    ...
    wordo = utilo.flat([[len(key)] * value for key, value in words.items()])
    result.word_avg = statistics.mean(wordo)
    result.sentence_avg = statistics.mean(sento)

    result.word_avg: float = utilo.roundme(result.word_avg)
    result.sentence_avg: float = utilo.roundme(result.sentence_avg)
    return result
