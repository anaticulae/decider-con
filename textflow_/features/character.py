...
def work(
    ...
    driver = protoerror.driver(sentences=sentences)
...
def check_character(linter, driver, regex):
    for page in utilo.flatten_content(driver.sentences):
        ...
        for sentence, pagenumber in zip(sentences, pagenumbers):
            ...
            for match in matched:
                ...
                linter(
                    location=location,
                    text=utilo.extract_match(match),
                )
...
SPACE_BEFORE_COMMA = utilo.compiles(r'\s+\,')


@protoerror.disable_perpage(morethan=MORETHAN)
...
MISSING_SPACE_AFTER_COMMA = utilo.compiles(r'\,[a-zA-Z]{3,}')
...
SPACE_BEFORE_SQUARE_BRACKET = utilo.compiles(r"""
...
SPACE_AFTER_SQUARE_BRACKET = utilo.compiles(r"""
...
SPACE_BEFORE_SEMICOLON = utilo.compiles(r'\s+\;')
...
MISSING_SPACE_AFTER_SEMICOLON = utilo.compiles(r'\;[^\s]')
...
MISSING_DOTS_BETWEEN_ANGLE_BRACKETS = utilo.compiles(r'\[\s*\]')
...
COMPARATIV_ALS = utilo.compiles(r"""
    (er)
    [ ]{1,3}
    (als)
    [ ]{1,3}
...
MISSING_MINUS_BETWEEN_VALUE_AND_SIGN = utilo.compiles(r'\d{1,4}(mal)')
...
SPACE_BEFORE_OR_AFTER_SLASH = utilo.compiles(r'([ ]\/[ ]|[ ]\/|\/[ ])')
...
SPACE_AFTER_PARAGRAPH = utilo.compiles(r'§\d+[a-z]?')
...
MISSING_MINUS_BETWEEN = utilo.compiles(r'\d{1,5}(jährig)(en){0,1}')
...
WRONG_ACCURACY = utilo.compiles(r'(etwa|circa|\bca\.?\b)[ ]{0,4}\d+')
...
HIGHNOTE_SPACE_BEFORE = utilo.compiles(r'[ ]{1,4}{{hn:\d{1,4}:nh}}')
...
HIGHNOTE_SPACE_TOO_MANY_AFTER = utilo.compiles(r'{{hn:\d{1,4}:nh}}[ ]{2,6}')
