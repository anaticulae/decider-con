...
class ReferenceMatcher:
    ...
    def add_ioi(self, page: int, number: str, raw: str = None):
        ...
        if number in [ioi.number for ioi in self.iois[page]]:
            utilo.error(f'duplicated ioi number: {number} on page {page}')
    ...
    def match(self):
        iois = utilo.flat([(page, value)
                           for value in values]
                          for page, values in self.iois.items())
        todos = utilo.flat([(page, value)
                            for value in values]
...
def create_matcher(references, iois) -> ReferenceMatcher:
    ...
    for ioi in utilo.flatten_content(iois):
    ...
    for reference in references:
        for start, raw in zip(reference.marked, reference.raw):
            ...
            if 'folgend' in test or 'unten' in test:
                ...
                if ref is None:
                    utilo.error(f'could not find forward ref: {reference}')
                    continue
            else:
...
def parse_reference(raw: str) -> str:
    ...
    if not parsed:
        utilo.error(f'invalid reference: {raw}')
        return None
    result = parsed[0]
    return result
